...
export itp, ramp, make_step, pulse, seasonal
...
"""
    ramp(times, time_units, start, finish, height=1.0)
...
# Examples
```julia
julia> r = ramp([0.0, 10.0], u"yr", 2.0, 5.0, 10.0)
...
"""
    pulse(times, time_units, start, height=1.0, width=1.0*time_units, repeat_interval=nothing)
...
# Examples
```julia
julia> p = pulse([0.0, 20.0], u"s", 5.0, 1.0, 2.0, 10.0)  # Pulse every 10s
...
"""
    seasonal(times, dt, period=u"1yr", shift=u"0yr")
...
# Examples
```julia
julia> wave = seasonal([0.0u"yr", 2.0u"yr"], 0.1u"yr", 1.0u"yr")
